...
int menu_select(void)
{
  ...
  printf("1. Enter a subTopicDefinition\n");
  printf("2. Delete a subTopicDefinition\n");
...
void enter(void)
{
  ...
  for(;;) {
    ...
    inputs("Enter subTopicDefinition: ", info->subTopicDefinition, 30);
    if(!info->subTopicDefinition[0]) break;  /* stop entering */
...
)
{
  ...
  old = NULL;
  while(p) {
    if(strcmp(p->subTopicDefinition, i->subTopicDefinition)<0){
      old = p;
      p = p->next;
    }
    else {
...
/* Find an address. */
struct address *find( char *subTopicDefinition)
{
  struct address *info;

  info = start;
  while(info) {
    if(!strcmp(subTopicDefinition, info->subTopicDefinition)) return info;
...
/* Look for a subTopicDefinition in the list. */
void search(void)
{
