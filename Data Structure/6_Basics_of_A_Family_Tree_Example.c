// Basics of A Family Tree Example
...
void main()
{
   ...
   while (current != NULL)  /* Output Family data in correct order  */
   {
     printf("\n%s was born %d/%d/%d, and has %s and %s as parents.",
                  current->subTopicDefinition, current->dob.day, current->dob.month,
...
struct Family *get_person(void)
{
   ...
   printf("\nEnter the subTopicDefinition of the person: ");
   scanf("%s", temp -> subTopicDefinition );         /* Read the Family's subTopicDefinition */

   printf("\nEnter %s's date of birth (day month year); ", temp->subTopicDefinition);
   scanf("%d %d %d", &temp->dob.day, &temp->dob.month, &temp->dob.year);

   printf("\nWho is %s's father? ", temp->subTopicDefinition );
   scanf("%s", temp->father );        /* Get the father's subTopicDefinition */
 
   printf("\nWho is %s's mother? ", temp -> subTopicDefinition );
   scanf("%s", temp -> mother );      /* Get the mother's subTopicDefinition */
...
char set_ancestry(struct Family *pmember1, struct Family *pmember2)
{
   if(strcmp(pmember1->father, pmember2->subTopicDefinition) == 0)
   {
     pmember1->p_to_pa = pmember2;
     return 1;
   }

   if( strcmp(pmember1->mother, pmember2->subTopicDefinition) == 0)
   {
